...
import { MapPin, Trophy, Calendar, Settings, LogOut, Edit2, Check, X, Loader2, Star, Award, TrendingUp, ChevronRight, Shield } from 'lucide-react';
...
export default function Profile({ onNavigate }: ProfileProps) {
  ...
  const badges = [
    { name: 'Top Scorer', icon: '🏆', desc: '10+ buts marqués', earned: userProfile.goals >= 10 },
    { name: 'Fidèle', icon: '💎', desc: '50+ matchs joués', earned: userProfile.matchesPlayed >= 50 },
    { name: 'Leader', icon: '👑', desc: 'Organisé 10+ matchs', earned: false },
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Buts', value: userProfile.goals, icon: Trophy, color: 'text-amber-400' },
          { label: 'Passes', value: userProfile.assists, icon: Trophy, color: 'text-blue-400' },
      ...
      <div className="grid md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                {isEditing ? (
                  <select 
                    ...
                    className="w-full mt-1 bg-dark-card border border-dark-border rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-500"
                  >
                    {['Débutant', 'Intermédiaire', 'Avancé', 'Pro'].map(l => <option key={l} value={l}>{l}</option>)}
                  </select>
                ) : (
        ...
        <div className="md:col-span-2 glass rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {badges.map((badge, i) => (
              <div key={i} className={`p-4 rounded-xl border-2 transition-all text-center ${
